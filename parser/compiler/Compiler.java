...
public class Compiler {
  ...
  void compileProgram() {
    try {
      ...
      DrawOffsetVisitor draw = new DrawOffsetVisitor(offsetVisitor.getMaxOffset(), offsetVisitor.getMaxDepth(), offsetVisitor.getNodeInfoTable());

      ast.accept(draw);

      try {
        File imagefile = new File(sourceFile + ".png");
        ImageIO.write(draw.getImage(), "png", imagefile);
      } catch (Exception e) {
      ...
      JLabel imagelabel = new JLabel(new ImageIcon(draw.getImage()));
      f.add("Center", imagelabel);
      f.pack();
      f.setSize(new Dimension(draw.getImage().getWidth() + 30, draw.getImage().getHeight() + 40));
      f.setVisible(true);
