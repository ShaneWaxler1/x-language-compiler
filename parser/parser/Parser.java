...
public class Parser {
    ...
    boolean startingDecl() {
        if (isNextTok(Tokens.Int) || isNextTok(Tokens.BOOLean) 
               || isNextTok(Tokens.Scientific) || isNextTok(Tokens.Char)) {
            return true;
        }
        return false;
    }
    ...
    boolean startingStatement() {
        if (isNextTok(Tokens.If) || isNextTok(Tokens.While) || isNextTok(Tokens.Return)
                || isNextTok(Tokens.LeftBrace) || isNextTok(Tokens.Switch) 
                || isNextTok(Tokens.Identifier) || isNextTok(Tokens.Unless)) {
            return true;
        }
        return false;
    }
        ...
        if(isNextTok(Tokens.Switch)){
            ...
            while(isNextTok(Tokens.Case)){
                ...
                if(isNextTok(Tokens.INTeger)){
                    c.addKid(new IntTree(currentToken));
                    scan();
                } else if(isNextTok(Tokens.CharLit)){
                    c.addKid(new CharTree(currentToken));
                    scan();
                }
                expect(Tokens.Colon);
                c.addKid(rStatement());
    ...
    public AST rFactor() throws SyntaxError {
        ...
        if(isNextTok(Tokens.ScientificLit)){  // -> <scientific>
            t = new ScientificTree(currentToken);
            scan();
            return t;
        }
        if(isNextTok(Tokens.CharLit)){      // -> <char>
